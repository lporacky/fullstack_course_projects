...
app.put("/jokes/:id", (req, res) => {
  ...
  const text = req.body.text;
  const type = req.body.type;
  const newJoke = jokes.find((j) => j.id === jokeId);
  newJoke.jokeText = text;
  newJoke.jokeType = type;
  res.json(newJoke);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const type = req.body.type;
  const newJoke = jokes.find((j) => j.id === jokeId);
  if (text) newJoke.jokeText = text;
  if (type) newJoke.jokeType = type;
  res.json(newJoke);
});
...
app.delete("/jokes/:id", (req, res) => {
  const jokeID = parseInt(req.params.id);
  jokes.delete(jokes.findIndex((j) => j.id === jokeID));
  res.json({ message: `Joke with id ${jokeID} has been deleted.` });
});
